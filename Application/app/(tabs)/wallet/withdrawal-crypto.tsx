...
export default function WithdrawCryptoScreen() {
  ...
  const { cUSDBalance, USDCBalance, totalBalance, address } =
    useLocalSearchParams();

  // Exchange rates (crypto to KES)
  const exchangeRates = {
    cUSD: 131.58, // 1 cUSD = 131.58 KES (approx)
    USDC: 131.58, // 1 USDC = 131.58 KES
  };
  ...
  const calculateKESAmount = () => {
    ...
    const rate = exchangeRates[selectedToken as keyof typeof exchangeRates];
                  ...
                  >
                    <View className="flex-row items-center flex-1">
                      ...
                      <View className="flex-1">
                        {method.id === "mpesa" ? (
                          <View>
                            <Image source={method.image} className="h-10 w-16"/>
                          </View>
                        ) : (
            ...
            {selectedMethod === "mpesa" && (
              <View className="bg-white p-6 rounded-2xl shadow-sm">
                ...
                <Text className="text-sm text-gray-500 mb-3">
                  Enter your M-Pesa registered number (To receieve Kes)
                </Text>
                  ...
                  <View className="">
                    <View className="flex-row justify-between items-center mb-3">
                      ...
                      <Text className="text-sm font-bold text-emerald-700">
                        1 {selectedToken} ={" "}
                        {exchangeRates[
                          selectedToken as keyof typeof exchangeRates
                        ].toFixed(2)}{" "}
                        KES
                      </Text>
                    </View>
